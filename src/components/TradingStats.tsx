  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {stats.map((stat, index) => {
        const isActiveInvestments = stat.title === 'Active Investments';
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <div className={`text-sm font-medium ${
                  ...
                  {stat.change}
                  {isActiveInvestments && activeOrders > 0 && ' • Clique para ver'}
                </div>
                {stat.change.includes('+') && (
        ...
        const baseClassName = `relative bg-gradient-to-br ${stat.bgColor} rounded-xl border ${stat.borderColor} p-6 hover:scale-105 transition-all duration-200 backdrop-blur-sm`;
        
        if (isActiveInvestments && activeOrders > 0) {
          return (
            <Link
              key={stat.title}
              to="/active-plans"

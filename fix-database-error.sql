-- Resolver erro "Database error saving new user"
...
-- 4. Criar função MUITO simples que NÃO falha
CREATE OR REPLACE FUNCTION create_user_profile_simple()
RETURNS TRIGGER AS $$
BEGIN
    -- Inserir perfil básico sem validações
...
-- 8. Verificar se há usuários na tabela auth.users
SELECT 
...
-- 9. Listar alguns usuários existentes
SELECT 
...
-- 10. Verificar se há perfis órfãos
SELECT 
...
-- 11. Remover perfis órfãos se existirem
DELETE FROM profiles 
WHERE user_id NOT IN (SELECT id FROM auth.users);

-- 12. Verificar se o admin souzamkt0 existe e tem perfil
...
-- 13. Criar tabela referrals se não existir
...
SELECT 
    ...
    'Teste o cadastro agora - erro deve estar resolvido' as status;
